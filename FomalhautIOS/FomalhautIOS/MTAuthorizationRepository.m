...
@implementation MTAuthorizationRepository

+ (MTAuthorizationRepository *)sharedInstance {
    static MTAuthorizationRepository *_sharedInstance = nil;
...
- (BOOL)store:(MTAuthorization *)auth {
    [self clear];
...
- (MTAuthorization *)load {
    ...
    if ([accounts count]) {
        ...
        return [MTAuthorization authorizationWithToken:password baseURL:[NSURL URLWithString:account]];
    }
    return nil;
}
